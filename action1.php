...
$objWorkshop = setId($_POST['wid ' ]);
		$objCart = new cart;
$workshops = $objWorkshop -> getWorkshopsById();
print_r($workshops):

$objcart = new cart;
$objCart-> setCid($_SESSION['cid']);
$objCart-> setPid($workshop['cid']);
$objCart -> setTitle($workshop['id']);
$objCart -> setQuantity(1);
$objCart -> setTotalAmount($workshop['price'];
$objCart-> setCreatedOn(date('y-m-d :i:s'));
break;
	...
	$data = jso_decode(base64_decode("users['token']), true);
	$currTime = strtotime("data['expTime']);
	if($currTime > $expTime){
	...
	$userData = $objUser ->getUserId();
	if(is_array($userData) && count($userDate) >0){
	...
	if(false ==$email){
	echo json_encode(["status"=> 0, "msg"=> "Password Updated."]);
	
	exit;
	}else{ echo json_encode(["status"=> 0, "msg"=>"user not found"]);
	exit;
	}
	$oobjUser -> setEmail($email);
	$objUser-> setEmail($email);
	$userData = $objUser-> getUserByEmakl();
if(is_array($userdata) && count($userData)>0){
$data['id'] = $userData['id'];
$data['token'] = shal($userData['email']);
...
$urlToken = base64_encode(json_encode($data));
	...
	$url ="http://'. $_server['server-name'].'/user/reset
